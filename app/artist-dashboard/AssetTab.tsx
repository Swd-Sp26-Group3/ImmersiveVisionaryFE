...
import {
  ...
  ShoppingBag, ArrowRight, Sparkles, X, Upload, Eye, AlertCircle,
} from "lucide-react";
    ...
    <Modal
      ...
      title={
        <span className="flex items-center gap-2">
          <Sparkles className="w-4 h-4 text-cyan-400" /> Create New Asset
        </span>
      }
      maxWidth="md"
      footer={
        <div className="flex gap-3">
          ...
          <Button
            onClick={handleSave}
            disabled={saving}
            className="flex-1 text-white rounded-xl font-semibold shadow-lg"
            style={{ background: "var(--gradient-accent)", boxShadow: "0 4px 20px rgba(6,182,212,0.2)" }}
          >
    ...
    >
      <div className="p-6 space-y-4 max-h-[55vh] overflow-y-auto">
        ...
        <div className="space-y-1.5">
          ...
          <textarea
            rows={3}
            placeholder="Describe this 3D/AR asset..."
            {...field("Description")}
    ...
    <div className="bg-slate-900/40 border border-white/[0.06] rounded-2xl overflow-hidden hover:border-cyan-500/20 transition-all group relative">
      ...
      <div className="p-4">
        ...
        {asset.PublishStatus === "DRAFT" && (
          <button
            onClick={() => onSubmit(asset.AssetId)}
            disabled={submitting}
        ...
        {isBlocked && (
          <p className="mt-2 text-[10px] text-yellow-500/60 leading-tight italic">
            Note: Original thumbnail blocked by source server. Showing fallback.
          </p>
        )}
      </div>
      ...
      {/* 3D Preview Modal */}
      <Modal
        open={showPreview && !!asset.Base64Data}
...
export function AssetsTab() {
  ...
  const handleSubmit = async (assetId: number) => {
    ...
    try {
      ...
      setAssets((prev) =>
        prev.map((a) => (a.AssetId === assetId ? { ...a, PublishStatus: "PENDING" } : a))
      );
      ...
      <div className="space-y-5">
        ...
        <div className="bg-blue-500/5 border border-blue-500/15 rounded-2xl p-4">
          ...
          <div className="flex items-center gap-2 flex-wrap text-xs">
            {(["Create Asset", "Submit for Review", "Manager Approves", "Live on Marketplace"] as const).map(
              (step, i, arr) => (
